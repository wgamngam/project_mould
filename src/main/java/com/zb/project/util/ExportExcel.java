...
public class ExportExcel<T> {
     ...
     * @param datas   需要显示的数据集合,集合中一定要放置符合javabean风格的类的对象。此方法支持的
    ...
    public void exportExcel(String title, String[] headers,
                            Collection<T> datas, OutputStream out, String pattern) {
        // 声明一个工作薄
        HSSFWorkbook workbook = new HSSFWorkbook();
        int size = AppConstants.EXPORTFILE_LENTH;
        ...
        int page = (int) Math.ceil((float)lenth/ size);
        for (int j = 0; j < page; j++) {
            List<T> dataset = new ArrayList<T>();
            int currIdx = j * size;
            for (int i = 0; i < size && i < lenth - currIdx; i++) {
                dataset.add(dataList.get(currIdx + i));
            }
            // 生成一个表格
            HSSFSheet sheet = workbook.createSheet("Sheet" + j);
            // 设置表格默认列宽度为15个字节
